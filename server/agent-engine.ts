...
export type AgentResult = { answer: string; provider: Provider; model: string; attempts: number; githubActions?: number };
export type AgentToolExecutor = (name: string, args: unknown) => Promise<unknown>;

type ToolCall = { id: string; type: "function"; function: { name: string; arguments: string } };
type ApiMessage = { role: "system" | "user" | "assistant" | "tool"; content: string | null; tool_call_id?: string; tool_calls?: ToolCall[] };
...
export const LIMITS = { promptChars: 4_000, historyMessages: 8, historyChars: 1_000, outputTokens: 384, timeoutMs: 15_000, maxCalls: 2, githubActionsPerTurn: 3, githubToolRounds: 3 } as const;
...
type Completion = { model?: string; choices?: Array<{ message?: { content?: string | null; tool_calls?: ToolCall[] } }> };
...
async function callProvider(fetcher: typeof fetch, url: string, key: string, model: string, messages: ApiMessage[], tools?: typeof githubTools) {
  let response: Response;
  try {
    response = await fetcher(url, {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model, messages, max_tokens: LIMITS.outputTokens, temperature: 0.2, stream: false, ...(tools ? { tools, tool_choice: "auto" } : {}) }),
  ...
  const toolCalls = Array.isArray(message?.tool_calls) ? message!.tool_calls!.slice(0, LIMITS.githubActionsPerTurn) : [];
  if (!answer && toolCalls.length === 0) throw new AgentError("INVALID_RESPONSE", "Der Modellanbieter lieferte weder Text noch einen Werkzeugaufruf.");
  return { answer: answer.slice(0, 20_000), toolCalls, model: (data.model || model).slice(0, 120) };
}
...
export async function runAgentTurn(input: AgentInput, allowFallback: boolean, deps: Dependencies = {}): Promise<AgentResult> {
  ...
  try {
    const result = await callProvider(fetcher, OPENROUTER_URL, primaryKey, "openrouter/free", makeMessages(input));
    return { answer: result.answer, provider: "openrouter", model: result.model, attempts: 1 };
  } catch (error) {
    ...
    const result = await callProvider(fetcher, HUGGINGFACE_URL, hfKey, HF_MODEL, makeMessages(input));
    return { answer: result.answer, provider: "huggingface", model: result.model, attempts: 2 };
  }
}

export async function runAgentTurnWithGitHub(input: AgentInput, executeTool: AgentToolExecutor, deps: Dependencies = {}) {
  const key = process.env.OPENROUTER_API_KEY?.trim();
  if (!key) throw new AgentError("MISSING_KEY", "Der OpenRouter-Schlüssel ist noch nicht sicher eingerichtet.");
  if (input.mode !== "workshop") throw new AgentError("REJECTED", "GitHub-Werkzeuge sind ausschließlich in der Projekt-Werkstatt verfügbar.");
  const fetcher = deps.fetcher ?? fetch;
  ...
  for (let round = 0; round <= LIMITS.githubToolRounds; round += 1) {
    const completion = await callProvider(fetcher, OPENROUTER_URL, key, "openrouter/free", messages, githubTools);
    selectedModel = completion.model;
    if (completion.toolCalls.length === 0) {
      return { answer: completion.answer || "Die Repository-Aktion wurde ausgeführt.", provider: "openrouter" as const, model: selectedModel, attempts: round + 1, githubActions: actions };
    }
    messages.push({ role: "assistant", content: completion.answer || null, tool_calls: completion.toolCalls });
    for (const call of completion.toolCalls) {
      let result: unknown;
      if (actions >= LIMITS.githubActionsPerTurn) {
        result = { ok: false, error: "Das Limit von drei GitHub-Aktionen pro Anfrage ist erreicht." };
      } else {
        try {
          const args = JSON.parse(call.function.arguments || "{}");
          if ((call.function.name === "github_write_file" || call.function.name === "github_open_pull_request") && !branchesCreatedThisTurn.has(args.branch)) {
            result = { ok: false, error: "Schreibzugriff ist nur auf einem Branch erlaubt, der in dieser Anfrage vom Agenten erstellt wurde." };
            messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify(result) });
            continue;
          }
          actions += 1;
          result = await executeTool(call.function.name, args);
          if (call.function.name === "github_create_branch" && typeof (result as { result?: { branch?: unknown } })?.result?.branch === "string") {
            branchesCreatedThisTurn.add((result as { result: { branch: string } }).result.branch);
          }
        } catch (error) {
          result = { ok: false, error: error instanceof Error ? error.message.slice(0, 500) : "Das GitHub-Werkzeug konnte nicht ausgeführt werden." };
        }
      }
      messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify(result).slice(0, 18_000) });
    }
    if (round === LIMITS.githubToolRounds) {
      return { answer: "Die begrenzte GitHub-Werkzeugrunde ist beendet. Ergebnisse: " + messages.filter((m) => m.role === "tool").map((m) => m.content ?? "").join("\n").slice(0, 8_000), provider: "openrouter" as const, model: selectedModel, attempts: round + 1, githubActions: actions };
    }
  }
  throw new AgentError("INVALID_RESPONSE", "Die GitHub-Werkzeugrunde konnte nicht abgeschlossen werden.");
}
...
export async function verifyOpenRouterKey(key: string, fetcher: typeof fetch = fetch): Promise<OpenRouterKeyStatus> {
  try {
    const response = await fetcher("https://openrouter.ai/api/v1/key", { method: "GET", headers: { Authorization: `Bearer ${key.trim()}` }, signal: AbortSignal.timeout(8_000) });
